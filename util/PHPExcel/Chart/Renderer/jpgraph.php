...
class PHPExcel_Chart_Renderer_jpgraph
{
    ...
    private function formatDataSetLabels($grouSetID, $datasetLabels, $labelCount, $rotation = '')
    {
        $datasetLabelFormatCode = $this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotCategoryByIndex(0)->getFormatCode();
    ...
    private function percentageSumCalculation($grouSetID, $seriesCount)
    {
        //    Adjust our values to a percentage value across all series in the group
        for ($i = 0; $i < $seriesCount; ++$i) {
            if ($i == 0) {
                $sumValues = $this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotValuesByIndex($i)->getDataValues();
            } else {
                $nextValues = $this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotValuesByIndex($i)->getDataValues();
    ...
    private function renderPlotLine($grouSetID, $filled = false, $combination = false, $dimensions = '2d')
    {
        $grouping = $this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotGrouping();

        $labelCount = count($this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotValuesByIndex(0)->getPointCount());
        if ($labelCount > 0) {
            $datasetLabels = $this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotCategoryByIndex(0)->getDataValues();
            $datasetLabels = $this->formatDataSetLabels($grouSetID, $datasetLabels, $labelCount);
            $this->graph->xaxis->SetTickLabels($datasetLabels);
        }

        $seriesCount = $this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotSeriesCount();
        $seriesPlots = array();
        if ($grouping == 'percentStacked') {
            $sumValues = $this->percentageSumCalculation($grouSetID, $seriesCount);
        }

        //    Loop through each data series in turn
        for ($i = 0; $i < $seriesCount; ++$i) {
            $dataValues = $this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotValuesByIndex($i)->getDataValues();
            $marker = $this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotValuesByIndex($i)->getPointMarker();
            ...
            $dataLabel = $this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotLabelByIndex($i)->getDataValue();
    ...
    private function renderPlotBar($grouSetID, $dimensions = '2d')
    {
        $rotation = $this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotDirection();
        //    Rotate for bar rather than column chart
        if (($grouSetID == 0) && ($rotation == 'bar')) {
        ...
        if ($labelCount > 0) {
            $datasetLabels = $this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotCategoryByIndex(0)->getDataValues();
            $datasetLabels = $this->formatDataSetLabels($grouSetID, $datasetLabels, $labelCount, $rotation);
            //    Rotate for bar rather than column chart
            if ($rotation == 'bar') {
        ...
        for ($j = 0; $j < $seriesCount; ++$j) {
            $dataValues = $this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotValuesByIndex($j)->getDataValues();
            if ($grouping == 'percentStacked') {
                $dataValues = $this->percentageAdjustValues($dataValues, $sumValues);
            }
            ...
            if (!$this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotLabelByIndex($j)) {
                $dataLabel = '';
            } else {
                $dataLabel = $this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotLabelByIndex($j)->getDataValue();
            }
            $seriesPlot->SetLegend($dataLabel);
    ...
    private function renderPlotScatter($grouSetID, $bubble)
    {
        $grouping = $this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotGrouping();
        $scatterStyle = $bubbleSize = $this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotStyle();
        ...
        //    Loop through each data series in turn
        for ($i = 0; $i < $seriesCount; ++$i) {
            $dataValuesY = $this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotCategoryByIndex($i)->getDataValues();
            $dataValuesX = $this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotValuesByIndex($i)->getDataValues();
    ...
    private function renderPlotRadar($grouSetID)
    {
        $radarStyle = $this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotStyle();
    ...
    private function renderPlotContour($grouSetID)
    {
        $contourStyle = $this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotStyle();
    ...
    private function renderPlotStock($grouSetID)
    {
        $seriesCount = $this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotSeriesCount();
        $plotOrder = $this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotOrder();

        $dataValues = array();
        //    Loop through each data series in turn and build the plot arrays
        foreach ($plotOrder as $i => $v) {
            $dataValuesX = $this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotValuesByIndex($v)->getDataValues();
    ...
    private function renderPieChart($groupCount, $dimensions = '2d', $doughnut = false, $multiplePlots = false)
    {
        ...
        for ($grouSetID = 0; $grouSetID < $iLimit; ++$grouSetID) {
            $grouping = $this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotGrouping();
            $exploded = $this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotStyle();
            if ($grouSetID == 0) {
                $labelCount = count($this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotValuesByIndex(0)->getPointCount());
                if ($labelCount > 0) {
                    $datasetLabels = $this->chart->getPlotArea()->getPlotGroupByIndex($grouSetID)->getPlotCategoryByIndex(0)->getDataValues();
    ...
    private function renderRadarChart($groupCount)
    {
        ...
        for ($grouSetID = 0; $grouSetID < $groupCount; ++$grouSetID) {
            $this->renderPlotRadar($grouSetID);
        }
    }
    ...
    private function renderStockChart($groupCount)
    {
        ...
        for ($grouSetID = 0; $grouSetID < $groupCount; ++$grouSetID) {
            $this->renderPlotStock($grouSetID);
        }
    }
