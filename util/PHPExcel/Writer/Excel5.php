...
class PHPExcel_Writer_Excel5 extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter
{
    ...
    private function buildWorksheetEschers()
    {
        ...
        foreach ($this->phpExcel->getAllsheets() as $sheet) {
            ...
            $spContainer->setSSetID(($sheet->getParent()->getIndex($sheet) + 1) << 10);
            ...
            foreach ($sheet->getDrawingCollection() as $drawing) {
                ...
                $reducedSSetID = $countShapes[$sheetIndex];
                $sSetID = $reducedSSetID
                    | ($sheet->getParent()->getIndex($sheet) + 1) << 10;
                $spContainer->setSSetID($sSetID);

                // keep track of last reducedSSetID
                $lastReducedSSetID = $reducedSSetID;

                // keep track of last sSetID
                $lastSSetID = $sSetID;
            ...
            $this->IDCLs[$dgId] = $lastReducedSSetID;

            // set last shape index
            $dgContainer->setLastSSetID($lastSSetID);
        ...
        foreach ($this->phpExcel->getAllsheets() as $sheet) {
            ...
            if (count($sheet->getDrawingCollection()) > 0) {
                ...
                foreach ($sheet->getDrawingCollection() as $drawing) {
                    ...
                    $sSetID = $sheetCountShapes | ($this->phpExcel->getIndex($sheet) + 1) << 10;
                    $sSetIDMax = max($sSetID, $sSetIDMax);
                }
            }
        }

        $dggContainer->setSSetIDMax($sSetIDMax + 1);
    ...
    private function writeDocumentSummaryInformation()
    {
        ...
        // GKSetIDDSI_VERSION :Version of the application that wrote the property storage
        $dataSection[] = array('summary'=> array('pack' => 'V', 'data' => 0x17),
                               'offset' => array('pack' => 'V'),
                               'type'     => array('pack' => 'V', 'data' => 0x03),
                               'data'    => array('pack' => 'V', 'data' => 0x000C0000));
        $dataSection_NumProps++;
        // GKSetIDDSI_SCALE : FALSE
        $dataSection[] = array('summary'=> array('pack' => 'V', 'data' => 0x0B),
                               'offset' => array('pack' => 'V'),
                               'type'     => array('pack' => 'V', 'data' => 0x0B),
                               'data'    => array('data' => false));
        $dataSection_NumProps++;
        // GKSetIDDSI_LINKSDIRTY : True if any of the values for the linked properties have changed outside of the application
        $dataSection[] = array('summary'=> array('pack' => 'V', 'data' => 0x10),
                               'offset' => array('pack' => 'V'),
                               'type'     => array('pack' => 'V', 'data' => 0x0B),
                               'data'    => array('data' => false));
        $dataSection_NumProps++;
        // GKSetIDDSI_SHAREDOC : FALSE
        $dataSection[] = array('summary'=> array('pack' => 'V', 'data' => 0x13),
                               'offset' => array('pack' => 'V'),
                               'type'     => array('pack' => 'V', 'data' => 0x0B),
                               'data'    => array('data' => false));
        $dataSection_NumProps++;
        // GKSetIDDSI_HYPERLINKSCHANGED : True if any of the values for the _SetID_LINKS (hyperlink text) have changed outside of the application
